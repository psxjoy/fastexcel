...
public abstract class AbstractWriteHolder extends AbstractHolder implements WriteHolder {
    ...
    /**
     * Execute the workbook handler chain Created in the sheet in the workbook interceptors will not be executed because
     * the workbook to create an event long past. So when initializing sheet, supplementary workbook event.
     */
    public WorkbookHandlerExecutionChain ownWorkbookHandlerExecutionChain;
    
    /**
     * Execute the sheet handler chain Created in the sheet in the workbook interceptors will not be executed because
     * the workbook to create an event long past. So when initializing sheet, supplementary workbook event.
     */
    ...
    public AbstractWriteHolder(WriteBasicParameter writeBasicParameter, AbstractWriteHolder parentAbstractWriteHolder) {
        ...
        if (writeBasicParameter.getCustomConverterList() != null && !writeBasicParameter.getCustomConverterList()
                .isEmpty()) {
    ...
    protected void initHandler(WriteBasicParameter writeBasicParameter, AbstractWriteHolder parentAbstractWriteHolder) {
        ...
        if (writeBasicParameter.getCustomWriteHandlerList() != null && !writeBasicParameter.getCustomWriteHandlerList()
                .isEmpty()) {
            handlerList.addAll(writeBasicParameter.getCustomWriteHandlerList());
        }
        if (Boolean.TRUE.equals(writeBasicParameter.getEscapeHex())) {
