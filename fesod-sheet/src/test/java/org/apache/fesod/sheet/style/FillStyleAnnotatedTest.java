...
public class FillStyleAnnotatedTest extends AbstractExcelTest {
    ...
    void fill(ExcelFormat format) throws Exception {
        ...
        if (format == ExcelFormat.XLSX) {
            try (XSSFWorkbook workbook = new XSSFWorkbook(new FileInputStream(file))) {
                ...
                fillXlsxCheck(sheet.getRow(1));
                fillXlsxCheck(sheet.getRow(2));
            }
        } else {
            try (HSSFWorkbook workbook = new HSSFWorkbook(new FileInputStream(file))) {
                HSSFSheet sheet = workbook.getSheetAt(0);
                fillXlsCheck(workbook, sheet.getRow(1));
                fillXlsCheck(workbook, sheet.getRow(2));
            }
        }
    }

    private void fillXlsxCheck(XSSFRow row) {
    ...
    private void fillXlsCheck(HSSFWorkbook workbook, HSSFRow row) {
    ...
    private void fill(File file, File template) throws Exception {
        FesodSheet.write(file, FillStyleAnnotatedData.class)
                ...
                .sheet()
                .doFill(TestDataBuilder.fillStyleAnnotatedData(10));
    }
